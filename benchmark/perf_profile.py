...
def run_perf(command, iterations=10):
    ...
    for i in range(iterations):
        try:
            # Запускаем perf stat
            subprocess.run(
                ["perf", "stat", "-j", "-e", "cpu_core/cycles/,cpu_atom/cycles/", "-o", "perf_tmp.json"] + command,
                check=True,
            ...
            for item in data:
                if item.get("event") == "cpu_atom/cycles/u":
                    atom_cycles = float(item["counter-value"].replace(',', '.'))
                elif item.get("event") == "cpu_core/cycles/u":
                    core_cycles = float(item["counter-value"].replace(',', '.'))
            
            if atom_cycles is not None and core_cycles is not None:
                total_cycles = int(atom_cycles) + int(core_cycles)
                cycles_data.append(total_cycles)
                print(f"Запуск {i+1}: {total_cycles:,} тактов (E-cores: {int(atom_cycles):,}, P-cores: {int(core_cycles):,})")
            else:
    ...
    if cycles_data:
        ...
        rmse = math.sqrt(mean_squared_error) / average_cycles
